...
export default function MasterCategoryPage() {
  ...
  const openDeleteModal = () => {
    if (selectedCategories.length === 0) {
      toast.error("Pilih kategori yang akan dihapus");
      return;
    }
  ...
  const handleDeleteSubmit = async () => {
    ...
    try {
      const payload = { ids: selectedCategories };
      const response = await api.delete("/category/delete", payload);
      if (response.status) {
        toast.success(`${selectedCategories.length} kategori berhasil dihapus`);
        setSelectedCategories([]);
  ...
  const allSelected = currentCategories.length > 0 && selectedCategories.length === currentCategories.length;
  const someSelected = selectedCategories.length > 0 && selectedCategories.length < currentCategories.length;
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-gray-50">
      <div className="px-6 py-8 space-y-6">
        <PageHeader
          ...
          subtitle="Pengelolaan data kategori"
            ...
            <div className="p-4 border-t border-gray-100">
              <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
                <div className="text-sm text-gray-500">
                  Menampilkan {indexOfFirstEntry + 1} sampai {Math.min(indexOfLastEntry, filteredCategories.length)} dari {filteredCategories.length} kategori
                </div>
              ...
              <div className="space-y-5">
                <div>
                  ...
                  <input
                    ...
                    required
                  />
                  <p className="text-xs text-gray-400 mt-1.5">Kategori utama (level 1)</p>
                </div>
              </div>
      ...
      {modalDetailOpen && selectedCategory && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 z-10">
            <div className="flex items-center justify-between p-6 border-b border-gray-200">
              <h2 className="text-xl font-semibold text-gray-800">Detail Kategori</h2>
              <button
                onClick={() => setModalDetailOpen(false)}
                className="text-gray-400 hover:text-gray-600 transition-colors rounded-lg p-1 hover:bg-gray-100"
              >
            ...
            <div className="p-6 space-y-4">
              <div className="flex justify-between items-center py-2 border-b border-gray-100">
                <span className="text-sm text-gray-500">Kode Kategori</span>
                <code className="text-sm font-mono bg-gray-100 px-2 py-1 rounded">
                  {selectedCategory.code}
                </code>
              </div>
              <div className="flex justify-between items-center py-2 border-b border-gray-100">
                <span className="text-sm text-gray-500">Nama Kategori</span>
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 z-10">
            <div className="p-6 text-center">
              ...
              <p className="text-gray-600 mb-6">
                Apakah Anda yakin ingin menghapus <strong>{selectedCategories.length}</strong> kategori yang
                dipilih?
                <br />
